...
temp_resnum_dict = { '1WM1' : { 'botA' : 138,#136,#138,
                                'capA' : 142,#140,
                                'capB' : 240,#238,
                                'botB' : 246},#248 },
                     '2WUF' : { 'botA' : 139,#139,
                                ...
                                'capB' : 216,#216,
                                'botB' : 222},#222 },
                     '3ANS' : { 'botA' : 361,#362,#361,
                                'capA' : 368,#369,
                                'capB' : 477,#474,
                                'botB' : 483},#485 },
                     '3B12' : { 'botA' : 128,#131,#128,
                                'capA' : 137,#139,
                                'capB' : 225,#226,
                                'botB' : 231},#230 },
                     '3GZJ' : { 'botA' : 111,#113,#111,
                                'capA' : 119,#121,
                                'capB' : 194,#195,
                                'botB' : 200}}#198 } }
...
for match in os.listdir('.'):
  if match.endswith('.massaged.pdb'):
    continue

  print 'will reorder: '+match
  pose_num = 1
  this_id = sys.argv[1][:4]
  ...
  with open(match, 'r') as pdb_in:
    ...
    for line in lines:
      ...
      if line[:4] == "ATOM":
        ...
        if linelist[21]=='A' and line[12:15]==' N ' and res_num==temp_resnum_dict[this_id]['botA']:
          botA_flag = 1
        if linelist[21]=='A' and line[12:15]==' N ' and res_num!=temp_resnum_dict[this_id]['botA'] and botA_flag==1:
  ...
  with open(match+'.massaged.pdb','w') as pdb_out:
...
for remodelready in os.listdir('.'):
  if remodelready.endswith('.pdbA.pdb'):
    continue
  if remodelready.endswith('.massaged.pdb'):
